    ...
    for (let provincias of datos) {
     //   console.log(provincias.ccaa);
        if (provincias.ccaa == comunidad) {
            ...
            col.dataset.lat = provincias.geo_point_2d.lat;
            col.dataset.lon = provincias.geo_point_2d.lon;

            col.addEventListener("click", mapituqui);
        }
    }
...
function mapituqui() {
    ...
    let urlMapa = "https://www.openstreetmap.org/#map=14/" + this.dataset.lat + "/" + this.dataset.lon;
